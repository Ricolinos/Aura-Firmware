...
- **Releases y actualizaciones selectivas (contrato v11).** Publicar un release = commit limpio + tag `vX.Y.Z` + `firmware/tools/package_dist.sh --release-tag vX.Y.Z` + GitHub Release con los assets exactos de la tabla §A del contrato (nunca `README.md` de `firmware/dist/`). No hay nada que "configurar" para que la actualización sea selectiva: Aura Studio calcula el delta solo, comparando los CRC32 del directorio central del `rockbox.zip` nuevo contra el manifiesto de lo instalado (`install_manifest.cfg`, que este firmware ignora — D-328). Lo único que este repo debe cuidar es **no romper la reproducibilidad del build**: nada de `__DATE__`/`__TIME__`, timestamps ni aleatoriedad en archivos generados que viajen dentro de `rockbox.zip` — entre releases consecutivos hoy cambian ~5 archivos de 9 431 (solo los de versión), y cada archivo que cambie sin necesidad vuelve más lenta la actualización de todos los usuarios. El release llega al iPod cuando **Aura-Studio** actualiza su pin (`FIRMWARE_VERSION`: `tag=` + hashes, `scripts/fetch-firmware.sh`) — trabajo del repo hermano, no de este.
...
- **La base tagcache es compartida entre familias (D-337, contrato v15)**: vive en `/.aura/tagcache/` (`AURA_SHARED_DB_DIR`, `aura_sync.h`) con su sello `db_stamp.txt`, nunca dentro de `/.rockbox/`. Ninguna ruta a `database_*.tcd` ni al sello se construye fuera de `aura_sync.c`; `apps/main.c` la fuerza en cada arranque antes de `tagcache_init()`. La caché de carátulas `cfcache/` sí es propia del árbol y se indexa por clave estable (D-338), no por `seek`.
