...
| `componentes/` | Componentes individuales con sus estados | 🟡 `StatusBar` casi cerrado; `LeftPanel`, `Selector`, `ScrollIndicator` con medidas exactas; `SelectionSummary` (rediseño D-267–D-272: fondo por acento, tipografía medida, sombra SDF) y `CoverDrift` con diseño y comportamiento definidos; `Music Flow` con investigación técnica y decisiones estructurales; `Movie Flow` (D-318) definido: copia casi exacta de Music Flow para Video — cartel 3:4, agrupamiento de series por temporada (`SxxEyy`), reverso de episodios en texto, fundido a negro al entrar al reproductor; `NowPlaying` CERRADO (2026-08-12; los dos valores que quedaban provisionales — stagger de entrada y umbrales de silencio del `LyricsPanel` — ratificados por el dueño en D-274); `LyricsPanel` (Modo 4) con spec propia completa; `SearchKeyboard` completo; `MarqueeText`, `DynamicTitle`, `ClockIndicator` definidos; `IndexRail` (riel A-Z de listas completas, D-276) definido; `About` (pantalla "Acerca de", D-279) definido: variante de `SelectionSummary` en split + estado expandido `FULL-CARRY` con `Shift-and-Reveal` (D-278); `PhotoViewer` (D-291, cuadrícula D-323) definido: cuadrícula de miniaturas + visor `FULL-COLD`, sonda de tamaño, `Fade-Slide` de región entre fotos; `DateEditor` con pantalla real (D-170) pero transición e ícono dinámico pendientes; patrón `InlineValue` (D-292, `left-panel.md`/`selector.md`) definido: fila con valor visible, sin submenú; `LibrarySync` (D-293) definido: pantalla `FULL` de progreso por sección al reconstruir la biblioteca tras un sync (marcador `/.aura/sync-pending.json`) o desde Ajustes — única pantalla completa de progreso del sistema, excepción documentada a la cápsula de espera |
| `transiciones/` | Vocabulario de patrones de animación reutilizables | 🟢 11 patrones documentados, los 11 implementados: `Morph Directo`, `Push-and-Drop`, `Lift-and-Push`, `Fade-on-Idle`, `Marquee Loop`, `Shift-and-Reveal` (D-278, primer consumidor `About`), `Fade-Slide`, `Scroll-Slide`, `Drop-and-Lift`, `Push-and-Pull` (300ms, D-274), `Flip-and-Flow` (500ms). Timings de Push/Drop ratificados y tokenizados (D-274) |
...
| `sistema/05-temas.md` | Sistema de temas (D-289): paquete instalable de fuentes+íconos+paleta en runtime, submenú "Temas" (D-292, ex-"Estilo"), fallback obligatorio al default; resolución del conflicto Modo↔Temas (D-292) | 🟢 Definido 2026-08-17 |
| `sistema/06-niveles-de-fx.md` | Índice transversal de los niveles de reducción de Animaciones/Gráficos (matriz del dueño 2026-08-18): regla de precedencia, dónde vive cada tabla por componente | 🟢 Definido 2026-08-18 |
