...
2. **El acento se gana.** El rosa solo aparece donde hay significado: íconos de menú y estado activo, la letra vigente del riel. Jamás como decoración de superficie. **Decisión explícita (AUDITORIA-01, sección 0-bis):** la fila seleccionada de una lista es un estado activo en este sentido — el texto y el ícono de la fila resaltada por la pastilla de selección van en `ACCENT`, no en `TEXT_PRIMARY`. No es una lectura literal de "la pastilla es gris neutro"; es la aplicación directa de este principio a la fila que el usuario tiene enfocada con la rueda en ese momento. Ver §5.1.
...
- **Supersampleo + composición por cobertura (AUDITORIA-01 A-01, supera D-075)**: `lcd_bitmap_transparent()` solo admite transparencia binaria por clave de color exacta (D-010) -- un pixel del bitmap ya generado es opaco o la clave magenta pura, sin mezcla parcial en tiempo de DIBUJO. Eso no obliga a que el asset mismo sea binario: el supersampleo de D-075 (8×, filtro LANCZOS, umbral de alfa a 128) mejoraba dónde caía el borde pero seguía decidiendo cada pixel todo-o-nada -- verificado por conteo real de tonos, no a ojo, los 468 iconos del sistema tenían exactamente 1 tono de tinta cada uno. La corrección real: renderizar a 16× el tamaño final, reducir con filtro de caja (cobertura real de subpixeles) y, en tiempo de GENERACIÓN, pre-componer la tinta de cada variante contra el fondo real del tema (`SHELL_BG` para la variante normal, `SELECTION_FILL` para `-on` -- su propio propósito documentado es la fila activa) usando la cobertura como máscara de mezcla. La clave magenta queda reservada solo para píxeles de cobertura exactamente cero; el resultado es un bitmap opaco de un solo plano, sin ningún cambio en `lcd_bitmap_transparent()` ni en el resto del código C, pero con una rampa real de tonos intermedios en el borde en vez de una escalera. El propio pipeline (`design-system/generate.py`) verifica esto mecánicamente: cuenta los tonos de tinta distintos de cada bmp generado y falla si alguno queda con menos de 4 -- no solo "compila", la rampa se confirma en cada regeneración. Limitación conocida: la composición fija un único fondo por variante, así que en los contextos donde `-on` se reutiliza sobre `SHELL_BG` directo (p. ej. Ahora suena) el borde queda pre-compuesto contra el fondo equivocado -- de todas formas muchísimo más fiel que la escalera binaria anterior, y el caso primario (fila resaltada) queda exacto. La vía de blending real en tiempo de dibujo (BMP de 32 bits con plano alfa + `lcd_alpha_bitmap_part`, que Rockbox sí soporta) sigue investigada y documentada, no implementada -- ver D-075.
- **Empaquetado en tiras únicas, sigue diferido**: el layout real de assets es un bmp por ícono/tamaño/variante en `design-system/out/icons/<tema>/`, no las dos tiras (`Apple2026Icons.bmp`/`...Dark.bmp`) que este documento menciona más abajo como optimización futura -- la composición por cobertura de arriba se aplica sobre este mismo layout de archivos sueltos, el empaquetado físico sigue siendo una mejora de IO independiente, sin fecha.
- **Siempre la variante lineal, nunca `.fill`, en la tira de íconos de menú.** El juego de íconos de menú es de trazo; una variante rellena al lado del resto se lee como una mancha maciza y rompe la fila. Si un símbolo solo existe relleno, se busca otro que signifique lo mismo. Excepciones documentadas, ninguna en la tira de menú: `star`/`star.fill` como control de valor (rating) en Ahora suena — ver `Reproductor - Ahora suena.md` §4; y `play.fill`/`pause.fill` en el botón central de transporte de Ahora suena (AUDITORIA-01, ambigüedad A-b) — la fila de transporte ya llevaba `backward.fill`/`forward.fill`, así que se unifica toda la fila a rellenos en vez de dejar el botón central lineal en medio de dos rellenos. El ícono de menú "Ahora suena" y el indicador de la barra de estado siguen usando `play`/`pause` lineales: no son controles de transporte.
...
- **Contenido de la fila activa: `ACCENT`, no `TEXT_PRIMARY`** ✅ *(decidido, AUDITORIA-01)*. Tanto el texto como el ícono de la fila resaltada se pintan en el color de acento del tema — es la aplicación directa del Principio 2 ("el acento se gana") a la fila que el usuario tiene enfocada con la rueda en ese momento, no una decoración de superficie. El relleno `SELECTION_FILL` sigue siendo neutro; lo que cambia de color es el contenido que va encima.
- Se desplaza entre filas con fundido/deslizamiento corto (ver Movimiento), nunca con un salto.
- Márgenes laterales, redactado sin ambigüedad: 8 px medidos desde el borde de la columna de lista (no desde el inset de 16 px del texto) — la pastilla queda más ancha que el texto pero nunca toca el borde de la columna ni el de la pantalla.
