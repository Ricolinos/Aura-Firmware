#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Reporte de uso de pila del hilo principal (D-345).
...
codigo de salida 1 -- si:
  ...
  (a) una funcion de `apps/aura/` supera --max-frame bytes (1024 por
      defecto), o
...
Uso:
    ...
    firmware/tools/stack_report.py --path aura_style_scan
...
`.su` por tres razones:
  ...
  1. Mide el binario que se empaqueta, no una recompilacion con otras
     ...
     existe como marco propio -- gcc la integra en `aura_style_scan`, y
...
ROOT = os.path.dirname(ROOT)  # .../Aura-Firmware

DEFAULT_BUILD = os.path.join(ROOT, "firmware", "build-ipod6g")
DEFAULT_OBJDUMP = os.path.join(ROOT, "firmware", "toolchain", "bin",
                               "arm-elf-eabi-objdump")
DEFAULT_NM = os.path.join(ROOT, "firmware", "toolchain", "bin",
                          "arm-elf-eabi-nm")

# Raices del hilo de UI. `main` es la de Rockbox; `aura_main` es la
# nuestra, y se lista aparte porque el camino que importa para el panic
# del dueno (D-345) nace ahi.
ROOTS = ("main", "aura_main")
...
GUARDED_EDGES = {
    ("skin_get_gwps", "skin_load"):
        ...
        "skins_initialised queda en false y skin_get_gwps() sale "
        "temprano para CUSTOM_STATUSBAR (skin_engine.c:298) -- la unica "
        "pantalla skinneable a la que Aura puede llegar. WPS y FM_SCREEN "
        "son pantallas de Rockbox que Aura reemplazo y nunca muestra.",
}
...
def aura_symbols(nm, build_dir):
    """Simbolos definidos en los .o de apps/aura/ (para separar el reporte)."""
    aura_dir = os.path.join(build_dir, "apps", "aura")
    syms = set()
    if not os.path.isdir(aura_dir):
        return syms
    objs = [os.path.join(aura_dir, f) for f in sorted(os.listdir(aura_dir))
            if f.endswith(".o")]
    if not objs:
        return syms
...
def main():
    ...
    ap.add_argument("--max-frame", type=int, default=1024,
                    help="marco maximo permitido en apps/aura/ (1024 B)")
    ...
    aura = aura_symbols(args.nm, args.build_dir)
    ...
    if not args.quiet:
        ...
        for group, title in ((True, "apps/aura/"), (False, "Rockbox base")):
            rows = [(v, k) for k, v in frames.items()
                    if (k in aura) == group and v > 0]
    ...
    print("stack_report: OK -- peor camino %d B (%.1f %% de %d B), "
          "ninguna funcion de apps/aura/ sobre %d B."
          % (peak, pct, stack_size, args.max_frame))
    return 0
