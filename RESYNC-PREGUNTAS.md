...
> **RESPONDIDO 2026-08-16 — ver D-274 en `DECISIONS.md`.** Resultado: C1, C2, C3, C5, C6, C8, C11, C12, C13, C14 → **ratificados** tal cual (🔴 cerrados en doc, TODO retirados del código). C4 → **300ms** (nuevo, antes 220). C7 → **300ms** (nuevo, antes 260). C10 → rosa unificado en **#FF2D55**. **C9 sigue pendiente** por decisión del dueño ("tengo que definir varias cosas"). **C15 → resuelta** en un segundo paso: el dueño aceptó la propuesta (1) `--layer-content` estrictamente 0 o 1 elemento por ahora, (2) `--layer-base` sí cambia de contenido sin transición de capa, siempre vía el debounce del panel derecho — reglas 5 y 6 de `sistema/01-capas-y-jerarquia.md`. **Único pendiente restante: C9.**
